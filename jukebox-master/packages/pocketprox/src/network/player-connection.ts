...
import { NetworkSession } from '@pocketprox/raknet'
import { FrameReliability } from '@pocketprox/raknet/lib/protocol/frame-reliability'

import { EntityPlayer } from '../entity/player'
import { pocketprox } from '../pocketprox'
...
export class PlayerConnection {
  ...
  public constructor(session: NetworkSession) {
    ...
    const world = pocketprox.getWorld()
    ...
    pocketprox.getServer().on('global_packet', this.sendPacketFn)
  ...
  private addWrapperToDecodingQueue(stream: BinaryStream): void {
    ...
    //  .catch(pocketprox.getLogger().error)
  }
  ...
  private handleLogin(login: McpeLogin): void {
    ...
    if (!Array.isArray(certChainData)) {
      this.disconnect()
      pocketprox.getLogger().error('Received an invalid McpeLogin certificate')
      return
    }
    ...
    if (pocketprox.getServer().getOnlinePlayer(playerData.displayName)) {
      // TODO: disconnect
    }
    ...
    if (pocketprox.getConfig().encryption != false) {
      const encryption = pocketprox.getEncryption()
      assert(
        encryption != null,
  ...
  private handleClientCacheStatus(packet: McpeClientCacheStatus): void {
    ...
    pocketprox.getLogger().debug(`Client cache support: ${this.cacheSupported}`)
  }
  ...
  private handleText(packet: McpeText): void {
    pocketprox.getServer().broadcastDataPacket(packet, true)
  }
  ...
  private handlePacketViolationWarning(
    ...
    pocketprox.getLogger().error(`PacketViolationWarning: ${packet.context}`)
  }
  ...
  public doInitialSpawn(): void {
    ...
    startGame.levelId = 'pocketprox Server'
    startGame.worldName = 'pocketprox Server'
    ...
    startGame.softwareVersion = 'pocketprox v0.0.1'
  ...
  public addToPlayerList(): WrapperPacket {
    ...
    pocketprox.getServer().broadcastDataPacket(sendSingleEntry(entry))
    const serverPlayerList = pocketprox.getServer().getPlayerList()
  ...
  public spawnClientSide(): void {
    ...
    const otherPlayers = pocketprox.getServer()
      .getOnlinePlayers()
  ...
  public disconnect(): void {
    ...
    pocketprox.getServer().removeListener('global_packet', this.sendPacketFn)
    this.player.close()
    // TODO
  }
}
